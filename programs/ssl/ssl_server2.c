                      ...
                      uint8_t *ik_pub, size_t *ik_pub_len)
{
    ...
    mbedtls_printf( "Public key dump for verification:\n%s\n", p_api->public_key_pem );

    mbedtls_printf( "Public key algorithm: %s\n", p_api->algorithm );
    
    earstatus = ear_jwt_verify( veraison_challenge_response_session->attestation_result,
                                (const uint8_t*) p_api->public_key_pem,
                                strlen(p_api->public_key_pem),
                                p_api->algorithm,
                                &ear,
                                ear_error_buffer );
